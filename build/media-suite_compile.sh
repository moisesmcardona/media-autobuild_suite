...
if [[ $standalone = y ]] && enabled libopus; then
    ...
    if do_vcs "https://github.com/xiph/opus-tools.git"; then
        ...
        do_uninstall "${_check[@]}"
		# Patch to assume output filename if it is not specified
		do_patch "https://patch-diff.githubusercontent.com/raw/xiph/opus-tools/pull/38.patch" am
        do_autogen
...
if [[ $ffmpeg != "no" ]] && enabled libmfx &&
    ...
    for _mfx_header in mfx{video,audio,plugin}++ mfx{adapter,pcp,scd,dispatcherprefixedfunctions}; do
        [[ -f "mfx/$_mfx_header.h" ]] || do_wget -r -c -q "https://raw.githubusercontent.com/Intel-Media-SDK/MediaSDK/master/api/include/$_mfx_header.h" \
            "mfx/$_mfx_header.h"
    done
    do_separate_confmakeinstall
    do_checkIfExist
